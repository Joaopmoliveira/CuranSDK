	...
	namespace ui
	{
		...
		curan::ui::callablefunction Panel::call()
		{
			auto lamb = [this](curan::ui::Signal sig, curan::ui::ConfigDraw *config)
			{
				...
				if (interpreter.check(MOUSE_UNCLICK_LEFT_EVENT))
				{
				...
				{
					if (!is_highlighting && !current_stroke.empty())
					{
						insert_in_map(current_stroke);
						current_stroke.clear();
					}
				...
				if (interpreter.check(MOUSE_CLICKED_LEFT_EVENT | INSIDE_FIXED_AREA))
				{
