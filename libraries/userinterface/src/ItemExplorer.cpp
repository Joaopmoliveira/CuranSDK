...
drawablefunction ItemExplorer::draw(){
	...
	auto lamb = [this](SkCanvas* canvas) {
		...
		while ((iterator != item_list.end()) && (y < widget_rect.fBottom))
		{
			...
			SkRect text_size = SkRect::MakeWH(0,0);
			font.measureText(iterator->text.data(),iterator->text.size(),SkTextEncoding::kUTF8,&text_size);
			
			canvas->drawSimpleText(iterator->text.data(),iterator->text.size(),SkTextEncoding::kUTF8,x-text_size.width()/2.0 + (ITEM_PREVIEW_WIDTH - iterator->cached_text_size.width()) / 2.0f,y + (iterator->cached_text_size.height()+ITEM_PREVIEW_HEIGHT - ITEM_IMAGE_HEIGHT)/2.0,font,text_paint);
