...
namespace curan
{
	namespace communication
	{
		...
		public:
			Socket(asio::io_context &io_context, const asio::ip::tcp::resolver::results_type &endpoints);
			...
			Socket(asio::io_context &io_context,
				   asio::ip::tcp::socket socket) : _cxt(io_context),
												   _socket(io_context),
												   is_connected{false},
												   timer{_cxt} {}
		...
		private:
			void lauch_start_after_connect(std::shared_ptr<Client<protocol>>)
			{
				is_connected = true;
				start(client);
			}

			void trigger_start(std::weak_ptr<Client> in_owner, const asio::ip::tcp::resolver::results_type &endpoints)
			{
				owner = in_owner;
			...
			void trigger_starttrigger_start(std::weak_ptr<Client> in_owner)
			{
				owner = in_owner;
