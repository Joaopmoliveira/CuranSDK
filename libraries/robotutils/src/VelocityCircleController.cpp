...
namespace robotic
{
    ...
    VelocityCircleController::VelocityCircleController(const std::string &model_file){
        {
    ...
    EigenState &&VelocityCircleController::update(kuka::Robot *robot, RobotParameters *iiwa, EigenState &&state, Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic>& composed_task_jacobians){
        ...
        // Then we compute the error between the current velocity and the desired velocity()

        Eigen::Vector3d velCurr = jacobianPos * state.dq;

        Eigen::Vector3d required_acceleration = ; 

        // We devide by the sample time and compute the acceleration required to 
