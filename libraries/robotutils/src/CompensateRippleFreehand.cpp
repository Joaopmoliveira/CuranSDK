...
namespace robotic {
    ...
    CompensateRippleFreehand::CompensateRippleFreehand(){
    ...
    EigenState&& CompensateRippleFreehand::update(kuka::Robot* robot, RobotParameters* iiwa, EigenState&& state, Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic>& composed_task_jacobians){
        ...
        */
        ...
        constexpr double proportional_gain = 1.5;
        constexpr double derivative_gain = 0.05;

        state.user_defined2[6] = derivative_torque[6];
        state.user_defined2[4] = derivative_torque[4];
        state.user_defined3[6] = filtered_derivative_torque[6];
        state.user_defined3[4] = filtered_derivative_torque[4];

        if(activate.load()){
            state.cmd_tau[6] = proportional_gain*state.user_defined[6]-derivative_gain*filtered_derivative_torque[6];
