...
int main() {
	try {
        ...
        constexpr unsigned int Dimension = 3;
        using RGBImageType = itk::Image<RGBAPixelType, Dimension>;

        using ReaderType = itk::ImageFileReader<RGBImageType>;
        ReaderType::Pointer reader = ReaderType::New();
        reader->SetFileName(CURAN_COPIED_RESOURCE_PATH"/images/cad_sentinel_pattern1_for_curan.png");
        try{
            reader->Update();
        } catch(...){
            return -1;
        ...
        VolumetricMask<RGBImageType> vol{reader->GetOutput()};
        std::unique_ptr<TwoDimensionalViewer<RGBImageType>> image_display_x = TwoDimensionalViewer<RGBImageType>::make(resources, &vol);
		...
		*container << std::move(image_display_x);
