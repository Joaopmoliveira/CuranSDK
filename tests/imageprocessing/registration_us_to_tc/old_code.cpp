...
// Function to generate six unique candidate rotation matrices by flipping signs of eigenvectors
std::vector<itk::Matrix<double, 3, 3>> GenerateRotationMatrices(const itk::Matrix<double, 3, 3>& rotationMatrix) {
    std::vector<itk::Matrix<double, 3, 3>> rotationMatrices;

    // Flip each axis individually
    for (int i = 0; i < 3; ++i) {
        itk::Matrix<double, 3, 3> modifiedMatrix = rotationMatrix;
        for (unsigned int k = 0; k < 3; ++k) {
            modifiedMatrix[k][i] = -rotationMatrix[k][i];
        }
        rotationMatrices.push_back(modifiedMatrix);
    }
    // Flip pairs of axes
    for (int i = 0; i < 3; ++i) {
        for (int j = i + 1; j < 3; ++j) {
            ...
            for (unsigned int k = 0; k < 3; ++k) {
                modifiedMatrix[k][i] = -rotationMatrix[k][i];
                modifiedMatrix[k][j] = -rotationMatrix[k][j];
            }
            rotationMatrices.push_back(modifiedMatrix);
        }
    }
    // Add the original matrix to the list
    rotationMatrices.push_back(rotationMatrix);
    // Ensure we have exactly six unique matrices
    std::vector<itk::Matrix<double, 3, 3>> uniqueMatrices;
    for (const auto& mat : rotationMatrices) {
        bool isUnique = true;
        for (const auto& uniqueMat : uniqueMatrices) {
            if (mat == uniqueMat) {
                isUnique = false;
                break;
            }
    ...
    // If we have more than six, reduce it to six by removing duplicates
    if (uniqueMatrices.size() > 6) {
        uniqueMatrices.resize(6);
    }

    return uniqueMatrices;
}

std::vector<itk::Matrix<double, 3, 3>> GenerateFlippedPrincipalComponents(const itk::Matrix<double, 3, 3>& principalComponents) {
    std::vector<itk::Matrix<double, 3, 3>> flippedMatrices;

    // Generate matrices by flipping the sign of each principal direction
    for (int i = 0; i < 3; ++i) {
        itk::Matrix<double, 3, 3> flippedMatrix = principalComponents;
        for (int j = 0; j < 3; ++j) {
            flippedMatrix[j][i] = -principalComponents[j][i];
        }
        flippedMatrices.push_back(flippedMatrix);
    }

    // Generate matrices by flipping the sign of each pair of principal directions
    for (int i = 0; i < 3; ++i) {
        for (int j = i + 1; j < 3; ++j) {
            itk::Matrix<double, 3, 3> flippedMatrix = principalComponents;
            for (int k = 0; k < 3; ++k) {
                flippedMatrix[k][i] = -principalComponents[k][i];
                flippedMatrix[k][j] = -principalComponents[k][j];
            }
            flippedMatrices.push_back(flippedMatrix);
        }
    }
...
int RegisterImages(const std::string& fixedImagePath, const std::string& movingImagePath, itk::Matrix<double, 3, 3>& rotationMatrix){
...
// 90-degree rotation around the X-axis
    ...
    Rx <<   1, 0, 0,
            0, 0, -1,
            0, 1, 0;
    //std::cout << "Rotation matrix around X-axis (90 degrees):\n" << Rx << "\n\n";
    // 90-degree rotation around the Y-axis
    Eigen::Matrix3d Ry;
    Ry <<   0, 0, 1,
            0, 1, 0,
           -1, 0, 0;
   // std::cout << "Rotation matrix around Y-axis (90 degrees):\n" << Ry << "\n\n";
    // 90-degree rotation around the Z-axis
    Eigen::Matrix3d Rz;
    Rz <<   0, -1, 0,
            1, 0, 0,
            0, 0, 1;
   // std::cout << "Rotation matrix around Z-axis (90 degrees):\n" << Rz << "\n\n";

/*
    auto flippedMatrices = GenerateFlippedPrincipalComponents(rotationMatrix);

         // Print the flipped matrices
    for (const auto& matrix : flippedMatrices) {
        PrintMatrix(matrix);
    }
*/

/*
    // Generate six possible initial conditions
    auto initialRotationMatrices = GenerateRotationMatrices(rotationMatrix);

     std::cout << "Candidate Rotation Matrices:" << std::endl;
    for (const auto& matrix : initialRotationMatrices) {
...
/*
    std::cout << "Centroid of Moving Point Cloud: " << movingCentroid << std::endl;
    std::cout << "Centroid of Fixed Point Cloud: " << fixedCentroid << std::endl;
    std::cout << "Principal Components of Moving Point Cloud: " << std::endl;
    std::cout << movingPCA << std::endl;
    std::cout << "Principal Components of Fixed Point Cloud: " << std::endl;
    std::cout << fixedPCA << std::endl;
...
int resampler(ImageType::Pointer moving, ImageType::Pointer fixed, itk::Matrix<double, 3, 3>& rotationMatrix, const std::string& output_path, itk::Point<double, 3U> moving_centroid, itk::Point<double, 3U> fixed_centroid){
    constexpr unsigned int Dimension = 3;
    using PixelType = float;
    using FixedImageType = itk::Image<PixelType, Dimension>;
    using MovingImageType = itk::Image<PixelType, Dimension>; 
 ...
 using TransformType = itk::VersorRigid3DTransform<double>;
    TransformType::Pointer transform = TransformType::New();
     ...
     // Convert the rotation matrix to a Versor
    using VersorType = TransformType::VersorType;
    VersorType versor;
    versor.Set(rotationMatrix);
    transform->SetRotation(versor);
    auto translation = fixed_centroid - moving_centroid;
 ...
 // Write result
    ...
    resampler(moving_volume, fixed_volume, rotationMatrix, output_transformed_image_path, moving_centroid, fixed_centroid);
