...
Application::Application(curan::ui::IconResources &in_resources, std::string path_to_load, std::mutex &inmut) : resources{in_resources},
                                                                                                                ...
                                                                                                                mut{inmut}
{
    ...
    map[PanelType::RESAMPLED_VOLUME].update_volume(nullptr);
    map[PanelType::TRAJECTORY_ORIENTED_VOLUME].update_volume(nullptr);
...
void Application::view_image_simple()
{
    ...
    if (button2 && button6)
        *viwers_container << std::move(button) << std::move(button2) << std::move(button4) << std::move(button6);
    else if (button2)
...
void Application::view_image_with_point_selection()
{
    ...
    auto text_container = Container::make(Container::ContainerType::LINEAR_CONTAINER, Container::Arrangement::VERTICAL);
    auto button_ac_point = (current_volume == PanelType::ORIGINAL_VOLUME) ? Button::make("ac point", "", resources) : Button::make("target", "", resources);
    button_ac_point->set_click_color(SK_ColorGRAY)
        .set_hover_color(SK_ColorLTGRAY)
        .set_waiting_color(SK_ColorDKGRAY)
        .set_size(SkRect::MakeWH(200, 200));
    auto button_pc_point = (current_volume == PanelType::ORIGINAL_VOLUME) ? Button::make("pc point", "", resources) : Button::make("plane xy", "", resources);
    button_pc_point->set_click_color(SK_ColorGRAY)
        .set_hover_color(SK_ColorLTGRAY)
        .set_waiting_color(SK_ColorDKGRAY)
        .set_size(SkRect::MakeWH(200, 200));
    auto button_midpoint = (current_volume == PanelType::ORIGINAL_VOLUME) ? Button::make("mid point", "", resources) : Button::make("entry", "", resources);
    ...
    auto perform_resampling = (current_volume == PanelType::ORIGINAL_VOLUME) ? Button::make("Resample to AC-PC", resources) : Button::make("Trajectory resampling", "", resources);
