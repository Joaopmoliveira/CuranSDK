...
int main(int argc, char **argv) {
    std::string filename5D =
        "../../../data/dynamics/pos-force-demo-gmm8-coupled-decoupledA-5D.json";
    vsgps::Dynamics dynamics = vsgps::Dynamics::fromJson(filename5D);
    Eigen::VectorXd xi(5), dxi_m(5), dxi_cpp(5);
    double e;
    int idx;
    bool thereWasError = false;
    std::ofstream logFile("log5D.log");
    ...
    for (auto const &l : data) {
        idx = &l - &data[0];
        xi = Eigen::VectorXd{{l.xi[0], l.xi[1], l.xi[2], l.xi[3], l.xi[4]}};
        dxi_m = Eigen::VectorXd{
            {l.dxi_m[0], l.dxi_m[1], l.dxi_m[2], l.dxi_m[3], l.dxi_m[4]}};
