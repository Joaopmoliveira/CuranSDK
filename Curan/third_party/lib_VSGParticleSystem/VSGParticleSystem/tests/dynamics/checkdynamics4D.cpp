...
int main(int argc, char **argv) {
    std::string filename4D =
        "../../../data/dynamics/pos-force-demo-gmm8-coupled-decoupledA-4D.json";
    vsgps::Dynamics dynamics = vsgps::Dynamics::fromJson(filename4D);
    Eigen::VectorXd xi(4), dxi_m(4), dxi_cpp(4);
    double e;
    int idx;
    bool thereWasError = false;
    std::ofstream logFile("log4D.log");
    ...
    for (auto const &l : data) {
        idx = &l - &data[0];
        xi = Eigen::VectorXd{{l.xi[0], l.xi[1], l.xi[2], l.xi[3]}};
        dxi_m =
            Eigen::VectorXd{{l.dxi_m[0], l.dxi_m[1], l.dxi_m[2], l.dxi_m[3]}};
