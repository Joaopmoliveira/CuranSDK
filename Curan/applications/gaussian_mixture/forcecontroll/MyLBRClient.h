...
cost of any service and repair. COPYRIGHT All Rights Reserved Copyright (C)
2014-2015 KUKA Roboter GmbH Augsburg, Germany This material is the exclusive
property of KUKA Roboter GmbH and must be returned to KUKA Roboter GmbH
immediately upon request. This material and the information illustrated or
contained herein may not be used, reproduced, stored in a retrieval system, or
transmitted in whole or in part in any way - electronic, mechanical,
photocopying, recording, or otherwise, without the prior written consent of KUKA
Roboter GmbH. \file \version {1.9}
*/
...
struct SharedState {
    ...
    std::atomic<float> sigma;
};

/**
 * \brief Template client implementation.
 */
class MyLBRClient : public KUKA::FRI::LBRClient, public LfD2LBR {
  ...
  private:
    ...
    struct RecordState {
        ...
        Eigen::Vector3d p0;
        Eigen::Vector3d dvel;
        Eigen::VectorXd torqueLinTask;
        Eigen::VectorXd torqueCommand;
        Eigen::VectorXd torqueConstrained;
    };

    void writeRecordState(RecordState const &rs);
    ...
    ToolData *myTool;
    // Vector3d pointPosition = Vector3d(0,0,0.071); // Point on center of
    // flange for MF-Touch
    Vector3d pointPosition =
        Vector3d(0, 0, 0.045); // Point on center of flange for MF-Electric
