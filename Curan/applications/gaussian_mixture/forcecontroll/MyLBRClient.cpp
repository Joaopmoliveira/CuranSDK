...
/**
 * @brief Computes the joint torque command and null space for a robot given its
 * current state and desired linear velocity.
 *
 * This function implements Operational Space Control (OSC) to calculate the
 * joint torque command that will move the robot towards a desired linear
 * velocity.
 *
 * @param sample_time The time step for the control loop.
 * @param jacobian The Jacobian matrix of the robot at its current
 * configuration.
 * @param massMatrix The mass matrix of the robot at its current configuration.
 * @param posCurr The current position of the robot's end-effector.
 * @param R_0_E The rotation matrix from the base frame to the end-effector
 * frame.
 * @param qDot The derivative of the joint angles, representing the current
 * joint velocities.
 * @param desLinVelocity The desired linear velocity of the robot's
 * end-effector.
 * @param desRotation The desired rotation of the robot's end-effector.
 * @param jointTorqueCommand A reference to a vector where the computed joint
 * torque command will be stored.
 * @param nullSpace A reference to a matrix where the null space of the Jacobian
 * will be stored.
 */
void computeLinVelToJointTorqueCmd(const double& sample_time, const Eigen::MatrixXd &jacobian, const Eigen::MatrixXd & massMatrix , const Eigen::Vector3d& posCurr , const Eigen::Matrix3d& R_0_E ,const Eigen::VectorXd &qDot, const Eigen::Vector3d &desLinVelocity, const Eigen::Matrix3d &desRotation,Eigen::VectorXd &jointTorqueCommand, Eigen::MatrixXd &nullSpace) //, std::ofstream &logRobot)
{
    ...
    const Eigen::MatrixXd jacobianPos = jacobian.block(0,0,3, NUMBER_OF_JOINTS); // jacobian(1:3,1:NUMBER_OF_JOINTS)
    const Eigen::MatrixXd jacobianRot = jacobian.block(3,0,3, NUMBER_OF_JOINTS); // jacobian(3:6,1:NUMBER_OF_JOINTS)
    ...
    auto maxCartSpeed = 0.4; // slack
    const double stiffness = 1000; // 400
    const double damping   = 44.71; // 40, aumentar, regra do polegar ser entre 15% a 20%
    ...
    /*
     * R_E_Ed is a 3x3 matrix representing the rotation error in the end-effector frame.
     * It is calculated by transposing the rotation matrix from the base frame to the end-effector frame (R_0_E)
     * and multiplying it with the desired rotation matrix (desRotation).
     * This results in a rotation matrix that transforms the desired rotation into the current end-effector frame.
     * This matrix is then converted to an axis-angle representation to compute the rotational error.
     */
    ...
    // Eigen::VectorXd torquePos = jacobianPos.transpose() * massMatrix * lambdaPos.inverse() * forcePos; // ?
    Eigen::VectorXd torqueRot = jacobianRot.transpose() * lambdaRot * forceRot;

    // Set torque command.
    // jointTorqueCommand = torquePos + nullSpaceTranslation * torqueRot;  // option 1 
    // jointTorqueCommand = torqueRot + nullSpaceRotation * torquePos;  // option 2 
    jointTorqueCommand = torquePos + torqueRot;                      // option 3 
    // jointTorqueCommand *= 0.;
...
void MyLBRClient::setOutputPath(std::string path) {
...
void MyLBRClient::setSharpness() {
    sharpness = (float) pipeline.getAdditionalOperations().gf.getSharpness(); 
    shared_state->sigma.store(sharpness);
}

template<typename V>
void vector2string(std::ostringstream &os, V v) {
    os << "[";
...
void MyLBRClient::writeRecordState(RecordState const &rs) {
    if (outputFile.is_open()) {
        os << "New record:\n";
        os << "\tt:\t\t " << rs.t << "\n";
        os << "\tp0:\t\t ";
        vector2string(os, rs.p0);
        os << "\n\tdvel:\t ";
        vector2string(os, rs.dvel);
        os << " with norm of " << rs.dvel.norm();
        os << "\n\ttorqueTask: ";
        vector2string(os, rs.torqueLinTask);
        os << "\n\ttorqueComm: ";
        vector2string(os, rs.torqueCommand);
        os << "\n\ttorqueCons: ";
        vector2string(os, rs.torqueConstrained);
        os << "End of record.\n\n";
    }
}

//todo: instead of json, save the data in a binary file (each byte can represent a value within a preconfigured structure)
...
void MyLBRClient::command() {
    ...
    iiwa->M(6, 6) *= 45; // * iiwa->M(6, 6); <== previous // Correct mass of last body to avoid large accelerations
    ...
    //pipeline.getAdditionalOperations().gf.setSharpness(shared_state->sigma.load());
    Eigen::Vector3d desired_velocity = applyDynamics(vsgps::State(p_0_cur)); // A*p_0_cur+b;
    std::cout << "Velocity's norm vector: " << desired_velocity.norm() << "\n";
    shared_state->velocity.store({desired_velocity[0],desired_velocity[1],desired_velocity[2]});
    
    Eigen::Matrix3d desired_orientation = Eigen::Matrix3d::Identity();
    desired_orientation << -0.994854, 0.0799006, -0.0623036,
                            0.0832716, 0.995088,  -0.0535279,
                            0.0577206, -0.0584406, -0.996621;
    ...
    Eigen::VectorXd torqueCommand = torqueLinTask + nullSpace*(dampingTorque+firstJointClamperTorque);
        
    // Limit torques to stop atwriteRecordState the robot's joint limits.
    ...
    writeRecordState({
        .t = currentTime,
        .p0 = p_0_cur,
        .dvel = desired_velocity,
        .torqueLinTask = torqueLinTask,
        .torqueCommand = torqueCommand,
        .torqueConstrained = SJSTorque
    });
