    ...
    for (size_t d = 0; d < image.depth(); ++d) {
        ...
        for (size_t r = 0; r < image.height(); ++r)
        {
            ...
            for (size_t c = 0; c < image.width(); ++c)
            {
                ...
                vsg::vec2  delta1((r_ratio - 0.5f), (c_ratio - 0.5f));
                float angle = atan2(delta.x, delta.y);
                //float angle2 = atan2(delta.y, delta.z);
                float distance_from_center = sqrt(c_ratio * c_ratio + r_ratio * r_ratio);
                //float distance_from_center1 = sqrt(d_ratio * d_ratio + r_ratio * r_ratio);
                float intensity = (sin(0.2 * angle + 30.0f * distance_from_center + 10.0 * value) + 1.0f) * 0.5f;
