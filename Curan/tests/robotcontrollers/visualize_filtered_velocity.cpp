...
void custom_interface(vsg::CommandBuffer &cb,curan::robotic::RobotLBR& client)
{
	...
	static std::array<ScrollingBuffer, LBR_N_JOINTS> buffers_vel;
    static std::array<ScrollingBuffer, LBR_N_JOINTS> buffers_filt_vel;
	static float t = 0;
	...
	if (ImPlot::BeginPlot("##Scrolling", ImVec2(-1, -1)))
	{
		...
		for (size_t index = 0; index < LBR_N_JOINTS; ++index)
		{
			...
			buffers_vel[index].AddPoint(t, (float)state.dq[index]);
            std::string filloc = "filt" + std::to_string(index);
            buffers_filt_vel[index].AddPoint(t, (float)state.user_defined[index]);
			ImPlot::PlotLine(loc.data(), &buffers_vel[index].Data[0].x, &buffers_vel[index].Data[0].y, buffers_vel[index].Data.size(), 0, buffers_vel[index].Offset, 2 * sizeof(float));
            ImPlot::PlotLine(filloc.data(), &buffers_filt_vel[index].Data[0].x, &buffers_filt_vel[index].Data[0].y, buffers_filt_vel[index].Data.size(), 0, buffers_filt_vel[index].Offset, 2 * sizeof(float));
        }
		ImPlot::EndPlot();
	}
