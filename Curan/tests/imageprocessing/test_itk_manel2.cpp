...
main(int argc, char * argv[])
{
  ...
  std::string dirName{CURAN_COPIED_RESOURCE_PATH"/itk_data_manel/BrainT1SliceBorder20.png"};
  fixedImageReader->SetFileName(dirName);

  std::string dirName2{CURAN_COPIED_RESOURCE_PATH"/itk_data_manel/BrainProtonDensitySliceShifted13x17y.png"};
  ...
  std::string Output1{"newmovedimage1.png"};
  ...
  resample->SetTransform(identityTransform);
    ...
    std::string Output2{"newmovedimage2.png"};
    writer->SetFileName(Output2);
    writer->Update();


  // After registration
  resample->SetTransform(registration->GetTransform());
    std::string Output3{"newmovedimage3.png"};
    writer->SetFileName(Output3);
    writer->Update();
